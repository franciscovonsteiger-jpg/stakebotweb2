...
def picks_para_usuario(user: dict) -> dict:
    """Filtra picks según el plan del usuario."""
    ...
    if plan in ("premium", "admin"):
        ...
        # Freemium: solo 3 picks, sin Gold Tips, sin stake detallado
        picks_vis = []
        for p in todos[:3]:
            picks_vis.append({**p, "stake_usd": None, "ganancia_pot": None,
                               "roi_diario_pct": None, "es_gold": False})
        gold_vis = []
        roi_vis  = None
        expo_vis = None
    ...
    if plan in ("premium", "admin") and bankroll != 1000:
        picks_ajustados = []
        for p in picks_vis:
            stake_aj  = round(bankroll * p.get("stake_pct", 0.02), 2) if p.get("stake_usd") else None
            gan_aj    = round(stake_aj * (p["odds_stake"] - 1), 2) if stake_aj else None
            roi_aj    = round(gan_aj / bankroll * 100, 2) if gan_aj else None
            picks_ajustados.append({**p, "stake_usd": stake_aj,
                                    "ganancia_pot": gan_aj, "roi_diario_pct": roi_aj})
        picks_vis = picks_ajustados
...
async def update_perfil(request: Request):
    user = require_auth(request)
    if not user:
        return JSONResponse({"ok": False}, status_code=401)
    data = await request.json()
    from core.database import update_perfil
    return JSONResponse(update_perfil(user["id"], data))
...
async def admin_set_plan(user_id: int, request: Request):
    ...
    set_user_plan(user_id, data["plan"])
    return JSONResponse({"ok": True})
...
async def admin_set_activo(user_id: int, request: Request):
    ...
    set_user_activo(user_id, data["activo"])
    return JSONResponse({"ok": True})
...
async def admin_invitaciones(request: Request):
    ...
    return JSONResponse(get_invitaciones())
...
<title>Stake Gold IA — Acceso</title>
<style>
:root{--bg:#0f1117;--bg2:#1a1d27;--bg3:#22263a;--border:#2e3348;--text:#e8eaf0;--text2:#8b92a8;--gold:#f59e0b;--green:#22c55e;--red:#ef4444;--purple:#8b5cf6;--radius:12px}
...
.btn-main{width:100%;padding:12px;background:var(--gold);border:none;border-radius:8px;color:#000;font-size:14px;font-weight:600;cursor:pointer;margin-top:8px}
...
<div class="card">
  <div class="logo">
    ...
    <div class="logo-sub">Señales profesionales para Stake.com</div>
  </div>
  ...
  <div id="form-login">
    ...
    <button class="btn-main" onclick="doLogin()" id="btn-login">Ingresar</button>
  ...
  <div id="form-register" class="hidden">
    <div class="plan-info">
      <strong>Plan Gratuito:</strong> 3 señales/día sin Gold Tips.<br>
      <strong>Plan Premium ($15,000 ARS/mes):</strong> Gold Tips completos + Telegram + ROI detallado.<br>
      Con código de invitación accedés directo al plan premium.
    </div>
    ...
    <button class="btn-main" onclick="doRegister()" id="btn-register">Crear cuenta</button>
...
async function doLogin(){
  ...
  if(d.ok){
    ...
    el.className='msg msg-err'; el.textContent=d.error||'Error al ingresar';
  }
}
...
async function doRegister(){
  ...
  const r=await fetch('/api/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({
      ...
      codigo:document.getElementById('r-code').value.toUpperCase(),
    })});
  const d=await r.json();
  ...
  if(d.ok){
    ...
    el.textContent=`Cuenta creada (plan ${d.plan}). Iniciá sesión.`;
    setTimeout(()=>showTab('login'),2000);
  } else {
    el.className='msg msg-err'; el.textContent=d.error||'Error al registrar';
  }
}
</script>
</body>
...
<style>
:root{--bg:#0f1117;--bg2:#1a1d27;--bg3:#22263a;--border:#2e3348;--text:#e8eaf0;--text2:#8b92a8;--gold:#f59e0b;--green:#22c55e;--red:#ef4444;--purple:#8b5cf6;--radius:10px}
*{box-sizing:border-box;margin:0;padding:0}
...
.card-title{font-size:14px;font-weight:600;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px}
...
.inv-code{font-family:monospace;font-size:18px;font-weight:700;color:var(--gold);letter-spacing:2px}
...
.msg{font-size:12px;color:var(--green);margin-top:6px}
</style>
</head>
<body>
<div class="topbar">
  <div class="logo">⭐ Stake Gold IA — Admin</div>
  <div style="display:flex;gap:8px">
    <button class="btn" onclick="window.location.href='/'">Ver dashboard</button>
...
<div class="container">
  <div class="metrics">
    ...
    <div class="metric"><div class="metric-label">Premium activos</div><div class="metric-val" id="m-premium" style="color:var(--gold)">—</div></div>
    <div class="metric"><div class="metric-label">Freemium</div><div class="metric-val" id="m-free">—</div></div>
    <div class="metric"><div class="metric-label">Invitaciones usadas</div><div class="metric-val" id="m-inv">—</div></div>
  </div>
  ...
  <div class="card">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
      <div class="field">
        <label>Plan</label>
        <select id="inv-plan"><option value="premium">Premium</option><option value="free">Free</option></select>
      </div>
      <div class="field">
        <label>Usos máximos</label>
        <input type="number" id="inv-usos" value="1" min="1" max="100" style="width:100px">
      </div>
    ...
    <div style="overflow-x:auto">
      <table>
        <thead><tr>
          ...
          <th>Bankroll</th><th>Telegram</th><th>Registro</th><th>Acciones</th>
        </tr></thead>
  ...
  <div class="card">
    <div class="card-title">Invitaciones generadas</div>
    <div style="overflow-x:auto">
      <table>
...
async function cargarDatos(){
  ...
  document.getElementById('tabla-usuarios').innerHTML = usuarios.map(u=>`
    <tr>
      ...
      <td>${u.tg_activo?'<span class="badge b-green">✓ Activo</span>':'<span class="badge b-gray">—</span>'}</td>
      <td style="font-size:11px;color:var(--text2)">${u.fecha_registro?u.fecha_registro.substring(0,10):''}</td>
  ...
  document.getElementById('tabla-invitaciones').innerHTML = invs.map(i=>`
    <tr>
      <td><code style="font-size:13px;color:var(--gold)">${i.codigo}</code></td>
      ...
      <td style="font-size:11px;color:var(--text2)">${i.fecha_creacion?i.fecha_creacion.substring(0,10):''}</td>
    </tr>`).join('');
}

async function generarInvitacion(){
  const plan   = document.getElementById('inv-plan').value;
  const usos   = parseInt(document.getElementById('inv-usos').value)||1;
  const r = await fetch('/api/admin/invitacion',{method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({plan,max_usos:usos})});
  const d = await r.json();
  if(d.ok){
    document.getElementById('inv-result').innerHTML=`
      <div class="inv-box">
        <div>
          <div style="font-size:11px;color:var(--text2);margin-bottom:4px">Código generado — compartí este link:</div>
          <div class="inv-code">${d.codigo}</div>
        </div>
        <button class="btn" onclick="navigator.clipboard.writeText('${d.codigo}');this.textContent='¡Copiado!'">Copiar código</button>
      </div>`;
    cargarDatos();
  }
}
...
async function cambiarPlan(id, plan){
  await fetch('/api/admin/usuario/'+id+'/plan',{method:'POST',
    headers:{'Content-Type':'application/json'},body:JSON.stringify({plan})});
  cargarDatos();
}

async function toggleActivo(id, activo){
  await fetch('/api/admin/usuario/'+id+'/activo',{method:'POST',
...
.b-free{background:var(--bg3);color:var(--text2)}
...
.btn-gold{background:var(--gold);color:#000;border-color:var(--gold);font-weight:600;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px}
.container{max-width:1100px;margin:0 auto;padding:20px 16px}
...
.freemium-banner{background:rgba(139,92,246,.08);border:1px solid rgba(139,92,246,.25);border-radius:var(--radius);padding:16px 18px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px}
...
.pick.bloqueado{opacity:.4;position:relative}
.lock-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;border-radius:var(--radius);backdrop-filter:blur(2px)}
...
<div class="topbar">
  <div class="logo">
    ...
    ⭐ Stake Gold IA
    <span id="plan-badge" class="badge b-gray">—</span>
  </div>
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
    ...
    <button class="btn" onclick="showPerfil()" id="btn-perfil">⚙ Mi perfil</button>
...
<div class="container">
  ...
  <div class="freemium-banner" id="freemium-banner" style="display:none">
    <div>
      <div style="font-weight:600;margin-bottom:4px">Estás en el plan gratuito</div>
      <div style="font-size:12px;color:var(--text2)">Ves 3 señales/día sin Gold Tips ni stake calculado.<br>Activá Premium para acceder a todo: Gold Tips, ROI, Telegram y más.</div>
    </div>
    <button class="btn-gold">⭐ Activar Premium — $15,000 ARS/mes</button>
  </div>
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="metric-label">Picks disponibles</div><div class="metric-val" id="m-validos">—</div><div class="metric-sub" id="m-bloq"></div></div>
  ...
  <div class="modal">
    ...
    <div class="field"><label>Perfil de riesgo</label>
      <select id="p-riesgo">
        ...
        <option value="inteligente">Inteligente — stake 3-5% (recomendado)</option>
        <option value="profesional">Profesional — Kelly completo</option>
      </select>
    </div>
    <div class="field"><label>Tu Chat ID de Telegram (para recibir Gold Tips)</label>
      <input type="text" id="p-tgid" placeholder="ej: 123456789">
    </div>
    <div class="field" style="display:flex;align-items:center;gap:10px">
      <input type="checkbox" id="p-tgactivo" style="width:auto">
      <label for="p-tgactivo" style="font-size:13px;color:var(--text)">Recibir Gold Tips por Telegram</label>
    </div>
    <div class="modal-btns">
      <button class="btn" onclick="hidePerfil()">Cancelar</button>
      <button class="btn-gold" onclick="guardarPerfil()">Guardar</button>
    </div>
...
async function loadUser(){
  ...
  USER=await r.json();
  const planLabels={'free':'Plan Gratuito','premium':'Plan Premium','admin':'Admin'};
  const planClasses={'free':'b-free','premium':'b-gold','admin':'b-green'};
  document.getElementById('plan-badge').textContent=planLabels[USER.plan]||USER.plan;
  document.getElementById('plan-badge').className='badge '+(planClasses[USER.plan]||'b-gray');
  ...
  return `<div class="pick">
    <div class="pick-top">
      ...
      <div>
        ...
        ${isPremium?`<div style="font-size:11px;color:var(--green);text-align:right;margin-top:2px">ROI ${fmtPct(p.roi_diario_pct)}</div>`:''}
      </div>
    </div>
    ...
    <div class="bar-labels">
      ...
      <span>Implícita: ${(1/p.odds_stake*100).toFixed(1)}%</span>
    </div>
    ${isPremium?`
    <div class="pick-bottom">
      ...
      <button class="btn" onclick="marcar(this)">✓ Colocado</button>
    </div>`:`
    <div style="padding-top:8px;border-top:1px solid var(--border);font-size:12px;color:var(--text3)">
      🔒 Stake y ROI disponibles en Plan Premium
    </div>`}
  </div>`;
}
...
function renderPicks(){
  ...
  if(DATA.picks_bloqueados>0&&USER?.plan==='free'){
    el.innerHTML+=`<div style="text-align:center;padding:20px;border:1px dashed var(--border);border-radius:var(--radius);color:var(--text2)">
      🔒 <strong>${DATA.picks_bloqueados} picks más</strong> disponibles en Plan Premium
      <div style="margin-top:8px"><button class="btn-gold" style="font-size:12px">⭐ Activar Premium</button></div>
    </div>`;
  }
}
...
function renderDescartados(){
  ...
  el.innerHTML=picks.map(p=>`<div class="pick" style="opacity:.45;border-style:dashed">
